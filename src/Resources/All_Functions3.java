...
public class All_Functions3 {
	...
	// Confirm page opens
		...
		// Excel Rows: Adjusting to Include EN HREF as the First Column
		public void excelRows(XSSFSheet sheet, int sectionCount, int TotalCount, int HeadN, Row rowHeading4) {
		    try {
		        ...
		        String[] hreflangLabels = {
		            ...
		            "JA HREF", "PT HREF", "RU HREF", "VI HREF", "CN HREF",
		            "AR HREF", "UK HREF"
		        };
	...
	public void getFileName(Row rowN, XSSFSheet sheet) {
	    try {
	        ...
	        WebElement FileName = wait.until(ExpectedConditions.visibilityOfElementLocated(
	            By.cssSelector(".active.carousel-item .PopupContent_multiMediaImageContainer__wunjw > div.PopupContent_imageCarousel__uy4Mq > span > img")
	        ));
	        ...
	        try {
	            ...
	            if (responseCode >= 200 && responseCode < 300) {
	                ...
	                throw new IOException("Image response code: " + responseCode);
	            }
	        } catch (IOException e) {
	...
	// Helper method to dynamically create HREF headers in the Excel sheet
	private void createHREFHeaders(XSSFSheet sheet, Row rowHeading4, List<WebElement> hrefElements) {
	    if (hreflangColumnMap == null) {
	        ...
	        for (WebElement hrefElement : hrefElements) {
	            ...
	            if (hreflang != null && !hreflangColumnMap.containsKey(hreflang.toLowerCase())) {
	                String header = hreflang.toUpperCase() + " HREF";
	                rowHeading4.createCell(HeadN).setCellValue(header);
	                hreflangColumnMap.put(hreflang.toLowerCase(), HeadN++);
	            }
	        }
	    }
	}

	// Helper method to add HREF links to a row dynamically
	private void addHREFLinksToRow(Row rowN, List<WebElement> hrefElements, XSSFSheet sheet) {
	    ...
	    for (WebElement hrefElement : hrefElements) {
	        ...
	        if (hreflang != null) {
	            hrefMap.put(hreflang.toLowerCase(), href);
	        }
	    }
